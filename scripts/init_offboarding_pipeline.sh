...
if [[ ! -f "$PIPELINE_STATE" ]]; then
  ...
  # --- RHOAI-only steps (pull pipelines, product listing) ---
  ...
  # These remove shared infrastructure used across all supported versions.
  # Only enable when the component is not needed by any supported version.
  GUARDED_STATUS="skipped"
  if [[ "$COMPONENT_EXISTS_IN_OLDER_VERSIONS" == "false" ]]; then
    GUARDED_STATUS="pending"
  fi

  # RHOAI-only guarded steps (delivery repo, product listing)
  ...
  # Auto-merge and renovate are RHOAI-only AND guarded
